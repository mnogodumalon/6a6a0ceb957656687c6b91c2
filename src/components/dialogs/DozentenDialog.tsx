...
export type DozentenDialogDefaults = Omit<Dozenten['fields'], 'fachbereiche' | 'beschaeftigungsart'> & {
    fachbereiche?: (LookupValue | string)[];
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  fachbereiche: LOOKUP_OPTIONS['dozenten']?.['fachbereiche'] ?? [],
...
export function DozentenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: DozentenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'email', 'beschaeftigungsart'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail-Adresse\n  "telefon": string | null, // Telefonnummer\n  "fachbereiche": LookupValue[] | null, // Fachbereiche / Instrumente (select one or more keys: "klavier" | "gitarre" | "schlagzeug" | "violine" | "cello" | "querfloete" | "saxophon" | "trompete" | "gesang" | "musiktheorie" | "komposition" | "sonstiges") mapping: klavier=Klavier, gitarre=Gitarre, schlagzeug=Schlagzeug, violine=Violine, cello=Cello, querfloete=Querflöte, saxophon=Saxophon, trompete=Trompete, gesang=Gesang, musiktheorie=Musiktheorie, komposition=Komposition, sonstiges=Sonstiges\n  "qualifikationen": string | null, // Qualifikationen\n  "beschaeftigungsart": LookupValue | null, // Beschäftigungsart (select one key: "festangestellt" | "honorar" | "ehrenamtlich") mapping: festangestellt=Festangestellt, honorar=Honorarbasis, ehrenamtlich=Ehrenamtlich\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "bemerkungen_dozent": string | null, // Bemerkungen\n}`;
      ...
      <div key="fachbereiche" className="space-y-1.5">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_klavier"
              checked={lookupKeys(fields.fachbereiche).includes('klavier')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
                  const next = checked ? [...current, 'klavier'] : current.filter(k => k !== 'klavier');
                  return { ...f, fachbereiche: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_gitarre"
              checked={lookupKeys(fields.fachbereiche).includes('gitarre')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_schlagzeug"
              checked={lookupKeys(fields.fachbereiche).includes('schlagzeug')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_violine"
              checked={lookupKeys(fields.fachbereiche).includes('violine')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
                  const next = checked ? [...current, 'violine'] : current.filter(k => k !== 'violine');
                  return { ...f, fachbereiche: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_cello"
              checked={lookupKeys(fields.fachbereiche).includes('cello')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_querfloete"
              checked={lookupKeys(fields.fachbereiche).includes('querfloete')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_saxophon"
              checked={lookupKeys(fields.fachbereiche).includes('saxophon')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
                  const next = checked ? [...current, 'saxophon'] : current.filter(k => k !== 'saxophon');
                  return { ...f, fachbereiche: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_trompete"
              checked={lookupKeys(fields.fachbereiche).includes('trompete')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_gesang"
              checked={lookupKeys(fields.fachbereiche).includes('gesang')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_musiktheorie"
              checked={lookupKeys(fields.fachbereiche).includes('musiktheorie')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_komposition"
              checked={lookupKeys(fields.fachbereiche).includes('komposition')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
                  const next = checked ? [...current, 'komposition'] : current.filter(k => k !== 'komposition');
                  return { ...f, fachbereiche: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="fachbereiche_sonstiges"
              checked={lookupKeys(fields.fachbereiche).includes('sonstiges')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.fachbereiche);
      ...
      <div key="beschaeftigungsart" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.beschaeftigungsart) === 'honorar'}
            onClick={() => setFields(f => ({ ...f, beschaeftigungsart: (lookupKey(f.beschaeftigungsart) === 'honorar' ? undefined : 'honorar') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.beschaeftigungsart) === 'honorar'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "fachbereiche": "Fachbereiche / Instrumente", "qualifikationen": "Qualifikationen", "beschaeftigungsart": "Beschäftigungsart", "eintrittsdatum": "Eintrittsdatum", "bemerkungen_dozent": "Bemerkungen"};
