...
export function ZahlungenDialog({ open, onClose, onSubmit, defaultValues, recordId, anmeldungenList, enablePhotoScan = true, enablePhotoLocation = true }: ZahlungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "anmeldung": string | null, // Display name from Anmeldungen (see <available-records>)\n  "rechnungsnummer": string | null, // Rechnungsnummer\n  "betrag": number | null, // Betrag (€)\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "bar" | "ueberweisung" | "sepa" | "online" | "sonstiges") mapping: bar=Barzahlung, ueberweisung=Überweisung, sepa=SEPA-Lastschrift, online=Online-Zahlung, sonstiges=Sonstiges\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "offen" | "bezahlt" | "teilbezahlt" | "erstattet") mapping: offen=Offen, bezahlt=Bezahlt, teilbezahlt=Teilbezahlt, erstattet=Erstattet\n  "notizen_zahlung": string | null, // Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'zahlungsart': (
      <div key="zahlungsart" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('zahlungen', 'zahlungsart', 'bar') ?? 'Barzahlung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsart) === 'sepa'}
            onClick={() => setFields(f => ({ ...f, zahlungsart: (lookupKey(f.zahlungsart) === 'sepa' ? undefined : 'sepa') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsart) === 'sepa'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zahlungen', 'zahlungsart', 'sepa') ?? 'SEPA-Lastschrift'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsart) === 'online'}
            onClick={() => setFields(f => ({ ...f, zahlungsart: (lookupKey(f.zahlungsart) === 'online' ? undefined : 'online') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsart) === 'online'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zahlungen', 'zahlungsart', 'online') ?? 'Online-Zahlung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsart) === 'sonstiges'}
            onClick={() => setFields(f => ({ ...f, zahlungsart: (lookupKey(f.zahlungsart) === 'sonstiges' ? undefined : 'sonstiges') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsart) === 'sonstiges'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zahlungen', 'zahlungsart', 'sonstiges') ?? 'Sonstiges'}
          </button>
        </div>
    ...
    'zahlungsstatus': (
      <div key="zahlungsstatus" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('zahlungen', 'zahlungsstatus', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
    ...
    'notizen_zahlung': (
      <div key="notizen_zahlung" className="space-y-1.5">
        <Label htmlFor="notizen_zahlung">{fieldLabel('zahlungen', 'notizen_zahlung')}</Label>
        <Textarea
          id="notizen_zahlung"
          placeholder=""
          value={fields.notizen_zahlung ?? ''}
          onChange={e => setFields(f => ({ ...f, notizen_zahlung: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"anmeldung": "Anmeldung", "rechnungsnummer": "Rechnungsnummer", "betrag": "Betrag (€)", "zahlungsdatum": "Zahlungsdatum", "zahlungsart": "Zahlungsart", "zahlungsstatus": "Zahlungsstatus", "notizen_zahlung": "Notizen"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"anmeldung": {"teilnehmer": "Teilnehmer", "kurs": "Kurs / Workshop", "anmeldedatum": "Anmeldedatum", "status_anmeldung": "Anmeldestatus", "bemerkungen_anmeldung": "Bemerkungen"}};
