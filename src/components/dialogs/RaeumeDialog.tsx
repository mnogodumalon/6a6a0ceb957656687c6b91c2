...
export type RaeumeDialogDefaults = Omit<Raeume['fields'], 'ausstattung'> & {
...
export function RaeumeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: RaeumeDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "raumname": string | null, // Raumname\n  "raumnummer": string | null, // Raumnummer\n  "etage": string | null, // Etage / Standort\n  "kapazitaet": number | null, // Kapazität (Personen)\n  "ausstattung": LookupValue[] | null, // Ausstattung (select one or more keys: "klavier" | "fluegel" | "schlagzeug" | "gitarrenverstaerker" | "pa_anlage" | "whiteboard" | "spiegel" | "tonstudio") mapping: klavier=Klavier, fluegel=Flügel, schlagzeug=Schlagzeug, gitarrenverstaerker=Gitarrenverstärker, pa_anlage=PA-Anlage, whiteboard=Whiteboard, spiegel=Spiegel, tonstudio=Tonstudio-Ausstattung\n  "bemerkungen_raum": string | null, // Bemerkungen\n  "verfuegbar": boolean | null, // Raum verfügbar\n}`;
    ...
    'ausstattung': (
      <div key="ausstattung" className="space-y-1.5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="ausstattung_gitarrenverstaerker"
              checked={lookupKeys(fields.ausstattung).includes('gitarrenverstaerker')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.ausstattung);
                  const next = checked ? [...current, 'gitarrenverstaerker'] : current.filter(k => k !== 'gitarrenverstaerker');
                  return { ...f, ausstattung: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="ausstattung_spiegel"
              checked={lookupKeys(fields.ausstattung).includes('spiegel')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.ausstattung);
                  const next = checked ? [...current, 'spiegel'] : current.filter(k => k !== 'spiegel');
                  return { ...f, ausstattung: next.length ? next as any : undefined };
                });
              }}
            />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="ausstattung_tonstudio"
              checked={lookupKeys(fields.ausstattung).includes('tonstudio')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.ausstattung);
                  const next = checked ? [...current, 'tonstudio'] : current.filter(k => k !== 'tonstudio');
                  return { ...f, ausstattung: next.length ? next as any : undefined };
                });
              }}
            />
  ...
  const FIELD_LABELS: Record<string, string> = {"raumname": "Raumname", "raumnummer": "Raumnummer", "etage": "Etage / Standort", "kapazitaet": "Kapazität (Personen)", "ausstattung": "Ausstattung", "bemerkungen_raum": "Bemerkungen", "verfuegbar": "Raum verfügbar"};
